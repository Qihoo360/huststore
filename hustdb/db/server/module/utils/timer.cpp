...
static int gen_timerfd(int second)
{
...
:m_interval(0), m_task_func(0), m_ctx(0)
{
}

timer_task_t::timer_task_t(int interval, task_func_t task_func, void * ctx)
:m_interval(interval), m_task_func(task_func), m_ctx(ctx)
{
}
...
{
    async_tasks_t() : m_pfds(0), m_size(0) {}
    ~async_tasks_t();

    bool init(const std::vector<timer_task_t>& tasks, int quit_fd);
    int wait(int timeout);
    bool quit();
    void run_tasks(std::vector<timer_task_t>& tasks);
...
bool async_tasks_t::init(const std::vector<timer_task_t>& tasks, int quit_fd)
{
...
int async_tasks_t::wait(int timeout)
{
...
void async_tasks_t::run_tasks(std::vector<timer_task_t>& tasks)
{
    for (size_t i = 0; i < m_size - 1; ++i)
    {
...
timer_t::timer_t() : m_tid(0), m_timeout(DEFAULT_TIMEOUT)
{
    pipe(m_stop_pipe);
...
bool timer_t::register_task(const timer_task_t& task)
{
...
void * timer_t::timer_thread(void * arg)
{
